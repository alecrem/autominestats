...
module.exports = class Fetcher {
  ...
  write_stats_jsons(timespans) {
    var parray = timespans.map(timespan => {
      ...
      .then(results => {
        fs.writeFile('data/stats_' + this.name + timespan.days + '.json', JSON.stringify(results, null, 2), (err) => {
          if (err) throw err;
  ...
  write_workers_jsons(timespans) {
    var parray = timespans.map(timespan => {
      ...
      .then(results => {
        fs.writeFile('data/workers_' + this.name + timespan.days + '.json', JSON.stringify(results, null, 2), (err) => {
          if (err) throw err;
